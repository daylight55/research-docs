...
このメモは日付順ではなく、発表資料を作るときに読みやすい順へ再編成している。先に結論とスライド構成を確認し、その後に基礎概念、比較、実装、仕様詳細、エコシステム、ロードマップ、制作メモへ進む。

## このメモの読み方

- まず「結論と発表構成」で、今回の発表で何を伝えるかを把握する。
- 次に「MCPの基礎理解」で、Host / Client / Server、Resources / Prompts / Tools、基本フローを押さえる。
- その後、「外部操作surface」「server設計」「Protocol/Auth」「Use case」「Roadmap」の順に深掘りする。
- スライド本文へ直接使う内容は前半、裏取りや詳細仕様は中盤以降、制作上の確認事項は末尾に置く。
...
発表を短くする必要がある場合、6つのmessageへ圧縮する。
...
deckはpure Q&Aにしない。core conceptは通常の説明storyで進め、engineerが反論しそうな箇所やdecision ruleが必要な箇所にQ&A slideを挿入する。

1. Title: MCP: AI agentと外部systemにはprotocolが必要
2. Why now: agentにはlive data、tool、workflow、governanceが必要
...
9. Multi-agent configuration: project/user/org settings、client schema差分、APM
10. Governance: LF/AAIF、Steering Group、SEP、RFC境界
11. History: Anthropic launchから現行spec/registryまで
12. Use cases: engineering、enterprise knowledge、business operations、agent platform
13. Ecosystem: GitHub、Stripe、OpenAI connector、registry、reference server
14. 開発で使うMCP: GitHub、Figma、Drive、Slack、Notion、Sentry、Firecrawl、Context7、OpenAI Docs
15. Skill + MCP: tool順序、検証、reportingをencodeし、反復workをreusable workflowへ変える
16. Quota-aware usage: plan/seat/rate-limit constraintの例としてFigma MCPを扱う
17. Security rules: approval、least privilege、no token passthrough、trusted servers
18. 自分たちに何が変わるか: 一度作って多くのclientへ接続できる。ただしproduction integrationとして運用する
19. Future points: registry、remote MCP、auth maturity、MCP Apps、durable tasks
20. Summary: MCPはgoverned agentic workのintegration layer

### 2026-06-09初心者視点レビューと構成見直し

対象聴講者を「LLMやAPIは知っているが、MCPを実装したことはない人」まで広げてレビューした。主要な問題は、MCPそのものの説明より前にAgent Skills、Remote設定、token効率、provider視点などが出て、初学者が何の地図を見ているかを失いやすい点だった。

#### 2026-06-09追加確認

2026-06-09時点の公式仕様ページでは、MCPのCurrent protocol versionは`2025-11-25`とされている。したがって、既存スライドの仕様参照を`2025-11-25`中心に置く前提は維持した。一方で、draftやroadmapには拡張仕様、Tasks、Apps、transport scalabilityなどの情報があるため、本編では詳細を追いすぎず「client supportとfallback確認が必要」という導入上の読み方に圧縮する。
...
| Page | Issue | Why it blocks beginners | Suggested fix / reflected change |
|---:|---|---|---|
...
| 6 | 章立てが情報の列挙になっていた | 聴講者が各章で何を得るかを予測できない | `本日の流れ`を「埋めるピース」と「ここでわかること」の表へ変更 |
...
以下は技術的には有用だが、初学者向け本編では理解の階段を壊しやすいため削除または研究メモ/参照リンク扱いにした。
...
#### 20+ iteration review log

| Iteration | Review lens | Finding | Change |
|---:|---|---|---|
| 1 | Overall story | 冒頭で地図がなく、用語列挙に見える | 概念図を冒頭へ追加 |
| 2 | One-sentence definition | MCPの定義が遅い | 一文定義スライドを追加 |
| 3 | Flow promise | 「何を順に埋めるか」が弱い | 本日の流れをピース表に変更 |
| 4 | First-principles order | 歴史と周辺エコシステムが基本部品より先に来る | Timeline / AI Agent潮流スライドを本編から削除 |
...
| 10 | Token load | Token比較が数値表と公開例で重複 | 公開例スライドを削除し、概念グラフへ集約 |
...
| 18 | Beginner density | 最新仕様やApps詳細が核の理解を邪魔する | 本編から詳細を削り、client support/fallbackだけ残す |
| 19 | Source stability | Current specが古い可能性を確認する必要 | 公式Current versionを追加確認し、調査メモへ追記 |
| 20 | Layout integrity | 新規概念図とdense化でoverflowが出る可能性 | Marp render + screenshot + DOM overflowで検証 |
| 21 | Final message | まとめが抽象的 | 「AI向けの安全な接続メニューを作る標準」に統一 |
| 22 | Appendix boundary | Referencesは本編理解には不要だが監査に必要 | Referencesはappendixとして残す |
...
#### Completion audit snapshot, 2026-06-09

| Requirement | Evidence | Status |
|---|---|---|
| 初学者が順番に理解できる | Slides 2-6で概念図、一文定義、結論、通し例、流れを提示。Slides 8-14で基本用語、1回の呼び出し、Skill/MCPの違いを説明。 | Proved by current deck structure |
| 抽象語や前提が説明されている | `agent`, `Host`, `Client`, `Server`, `Tool`, `Resource`, `Prompt`, `Protocol`, `Transport`, `Auth`を本編前半で定義。レビュー表で抽象語の言い換えを記録。 | Proved by slide text and review notes |
| 中級者にも骨太な理解がある | Slides 28-33でMCP server設計、アンチパターン、route policy、description設計を扱う。Slides 39-49でtransport/auth/JSON-RPC/tool-call境界を扱う。Slides 65-68でtrust boundary、server設計ルール、context効率を扱う。 | Proved by coverage in current deck |
| 冒頭の概念図からピースを埋める流れ | Slide 2に概念図、Slide 6に埋めるピース表。各章がHost/Client/Server、Tool/Resource/Prompt、Transport/Auth、Workflow/Governanceへ対応。 | Proved by slide sequence |
| 冗長・変動しやすい情報が整理されている | GitHub stars、Figma rate limit具体値、MCP Apps/Tasks詳細、長いCLI設定例を本編から削除し、レビュー記録にcut candidatesを明記。 | Proved by diff and review notes |
| 調査情報が追跡できる | Current specification確認、source links、research memo、References appendixを保持。 | Proved by research memo and references slides |
...
| レンダリング品質 | `npm run slide:mcp:verify`で76 screenshot生成。Playwright DOM overflow check returns `[]`。 | Proved by command output |
| repo validation | `npm test` passes 4 tests. `npm run build:site` builds 6 pages and deck HTML. | Proved by command output |
...
### 初心者向け詳細メモ, 2026-06-07

このsectionは、最終発表のための初心者向けsource material。実装詳細へ進む前に、MCPの下にある概念を意図的に説明する。
...
スライドで強調する点: OpenAPI生成は出発点にすぎない。production MCP serverでは、すべてのendpointを無差別に公開してはいけない。model-facing tool surfaceをcurateする必要がある。
...
これらはslide-levelの例として使う。意図的に短くしているため、実際のproduction configではversion pin、scope、policy、observability、secret handlingを追加する。
...
Microsoft Agent Package Manager (APM)は、それ自体がMCP clientではない。agent context向けのdependency managerである。この発表で有用なのは、`apm.yml`をskills、prompts、instructions、plugins、MCP serversのsingle source of truthにでき、`apm install`がGitHub Copilot、Claude Code、Cursor、Codex、Gemini、OpenCode、Windsurf向けのruntime-specific fileを書き出す、という点。
...
- Figma: design context、screenshot、design-to-code、deck/diagram workflow。
...
### 現状、ロードマップ、論争、非developer領域への拡大, 2026-06-08
...
revised deckでは、MCPをstandaloneのdeveloper convenienceではなくgoverned external-action layerとして位置づける。最も強いsynthesisは次の通り。
...
slide上の含意: early slideの「Skill = how to proceed / MCP = what to access or execute」というframeは維持しつつ、operationに落とす。PR repairやdesign-to-code workが有用な例になる。
...
| seat / plan例 | Figma docs上のlimit pattern | slideでの意味 |
|---|---|---|
...
1. **Stable core specification: MCP 2025-11-25.** 公式specification pageでは2025-11-25がlatest core versionとされている。このdeckで重要な追加点は、URL mode elicitation、sampling with tools、JSON Schema 2020-12 default、icons/metadata、structured output/resource links、authorization discovery improvement、experimental tasks。
...
slide上の含意: "MCP now supports UI everywhere"とは言わない。正確なclaimは次の通り。
...
##### slide向けlatest feature matrix

| feature | specification status | deckでの扱い |
|---|---|---|
...
現在の推奨Marp practiceは、slide contentをMarkdownに保ち、visual identityをplain CSSへ移すこと。

- deck-wide metadataには、Marp front matterの`theme`、`paginate`、`size`、`title`、`footer`を使う。
- reusable visual designが必要な場合は、`/* @theme ... */` meta comment付きのcustom Marpit theme CSS fileを使う。
- slideごとのlayout variationには、`_class: lead`、`_class: section`、`_class: dense`、`_class: rank`のようなlocal class directiveを使う。
- CSS Grid/Flexbox、design token、`text-wrap: balance/pretty`、bounded typography、控えめなshadow/borderを優先する。
- repoが明示的にbuild artifactを必要としない限り、生成HTML/PDFはsource repoに入れない。代わりに再現可能な`marp-cli` commandをdocumentする。
- HTML exportと代表PNG slide exportでdeckをverifyする。

このdeckで適用したdesign decision:

- reusable custom Marp themeとして`contents/themes/mcp-modern.css`を追加した。
- Markdown sourceをprimary artifactとして維持した。
- Frontend、Cloud、Developer MCPsのsection-divider slideを追加した。
- すべてのslideを質問形式にせず、narrative explanation slideとtargeted Q&A slideを混ぜた。
- slideごとのinformation densityを制御するため、`compact`、`dense`、`split`、`cards`、`rank` classを追加した。
- single-hue themeではなく、teal、vermilion、violet accentを使う落ち着いたtechnical paletteを使った。
- visual inspection用に`marp-cli` HTML exportとPNG exportでverifyした。

### 最終deckへの確認事項

- audience level: general internal audienceか、engineer-heavyか。
- length: 10分、20分、45分のどれか。
- exampleはCodex workflow、Claude/Cursor workflow、company-wide agent platformのどれに寄せるか。
- GitHub/Figma/Sentry/Firecrawl MCP usageのようなlive demoを含めるか。
