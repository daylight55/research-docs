...
The GitHub Pages workflow deploys `main` to the root site and publishes pull request previews under `pr-preview/pr-<PR_NUMBER>/`. GitHub Pages must be enabled for the repository with `gh-pages` as the source branch and `/` as the source path. Deployments share one queue and wait for the corresponding Pages build to finish before the next `gh-pages` update starts, preventing a newer deployment from cancelling one that is still in progress.
