...
外部の技術プレゼンテーション資料を確認した結果、今回のMCP資料は「定義から始める」よりも「問題と具体例から始め、必要になったところで定義する」方が自然だと判断した。

- MIT Communication Labは、技術発表の成功基準として、研究の動機になる問題と、なぜそれを提示するのかから始めることを挙げている。また、すべてのスライドはその動機づけ問題に関係し、必要以上の情報を載せないべきだとしている。
- Michael Ernstの技術発表ガイドは、タイトル直後にoutlineから入らず、解いている問題の例など動機づけから始めることを勧めている。これは、今回の冒頭で全体地図やMCP定義を先に置く構成への反証になる。
...
1. **判断できるようになること**: MCP用語ではなく、agentに外部systemを任せるときの判断軸を先に置く。
2. **失敗CIの通し例**: 読者が実務上の困りごととして外部接続を捉えられるようにする。
3. **外部systemへ出る入口**: LLM単体では現在のGitHub、DB、SaaS、AWSを扱えないことを示す。
4. **MCPの全体地図**: 具体例を地図に載せ、User -> Host -> MCP Server -> Backendの主線を見る。
5. **MCPは接続契約**: 問題と地図を見た後で、MCPの定義を置く。
6. **全体の流れ**: 章立てをoutlineではなく、判断軸を作る順番として示す。

この並びにより、初学者は「MCPとは何か」を暗記する前に、「なぜMCPという接続契約が必要になるのか」を理解できる。

#### 初学者レビュー表

| 対象箇所 | Issue | Why it blocks beginners | Suggested fix / reflected change |
|---:|---|---|---|
| 2 | 旧構成では全体地図が早すぎる | 何の困りごとを解く地図なのか分からず、部品名の暗記に見える | `判断できるようになること`を追加し、最初に判断軸を置く |
| 3 | 旧構成ではMCP定義が早すぎる | 問題例を持たないまま「接続契約」と言われても必要性が弱い | `失敗CIを調べる`例を先に置き、外部systemへ出る必要を具体化する |
| 4 | 外部接続の前提が弱い | LLMがなぜGitHub、AWS、DBを直接扱えないのかが曖昧 | `外部systemへ出ると何が起きるか`でHost境界と接続面を図示する |
| 5 | 地図が暗記用に見える | User -> Host -> Server -> Backendの主線より、周辺ピースに目が行く | CI例を地図に載せ、左から右の流れだけを先に見るよう説明する |
| 6 | 一文定義がスローガン化する | 「共通ルール」だけでは何を標準化するか分からない | `MCPは接続契約`として、Host、Client、Server、Backendの役割へ分解する |
