...
対象読者を「LLMやAPIは知っているが、MCPを実装したことはない人」まで広げてレビューした。主要な問題は、MCPの名前や構成要素を早く出すことではなく、読者が「なぜこの接続面を考える必要があるのか」を持つ前に、地図・定義・用語分類を受け取ってしまう点だった。
...
外部の技術プレゼンテーション資料を確認した結果、今回のMCP資料は「定義から始める」よりも「全体地図を先に置き、その地図の読み方を固定してから細部へ進む」方が自然だと判断した。MCPはHost、Client、Server、tool、transport、authなどの用語が多いため、個別用語より先に全体配置を見せる価値が高い。一方で、地図直後に具体例へ飛ぶと、読者はまだ地図を読めないため、例は後段の比較章で扱う。

- MIT Communication Labは、技術発表の成功基準として、研究の動機になる問題と、なぜそれを提示するのかから始めることを挙げている。また、すべてのスライドはその動機づけ問題に関係し、必要以上の情報を載せないべきだとしている。今回の資料では、動機を長い例で出すより、MCPが置かれる全体地図を先に置く方が、後続の用語の受け皿になる。
- Michael Ernstの技術発表ガイドは、タイトル直後にoutlineから入らず、解いている問題の例など動機づけから始めることを勧めている。ここから、冒頭で単なる目次表から入らないこと、また地図を出す場合も「何を見る地図か」を明確にする必要があると解釈した。
- Google Technical Writing Oneは、対象読者、学習目標、読者が何を知っていて何を学ぶ必要があるかを明確にすることを重視している。MCP資料では「読者がMCP用語を知らない」ことだけでなく、「外部systemをagentに任せる判断軸をまだ持っていない」ことを前提にする必要がある。

この確認に基づき、冒頭は次の順に再編した。

1. **MCPの全体地図**: 細部を読ませず、User -> Host -> MCP Server -> Backendの主線だけを見る。
2. **地図の主線**: Host、MCP Server、Backendの責務境界を先に固定する。
3. **MCPは接続契約**: 地図上のどの境界を標準化するのかを定義する。
4. **外部systemへ出る入口**: LLMがHostの中で判断し、外部systemには接続面を通じて届くことを図で補強する。
5. **全体の流れ**: 地図のピースをどの順番で埋めるかを示す。

この並びにより、初学者は「MCPとは何か」を暗記する前に、まずMCPが置かれる場所を理解できる。具体例はこの段階では出さず、比較章でCLI / Browser / MCPを比べるタイミングに回す。

#### 初学者レビュー表

| 対象箇所 | Issue | Why it blocks beginners | Suggested fix / reflected change |
|---:|---|---|---|
| 2 | 全体地図の前に判断軸を置くと抽象的 | まだ地図を見ていないため、何を判断する話なのか分からない | 冒頭を`MCPの全体地図`に戻し、主線だけを見る説明にする |
| 3 | 具体例が早すぎる | MCPの配置を理解する前にCI調査が出るため、何の例なのか分からない | 00章から失敗CI例を外し、比較章の利用フローへ寄せる |
| 4 | 地図が暗記用に見える | User -> Host -> Server -> Backendの主線より、周辺ピースに目が行く | `地図の主線`を追加し、Host / MCP Server / Backendだけを先に説明する |
| 5 | 一文定義がスローガン化する | 「共通ルール」だけでは何を標準化するか分からない | `MCPは接続契約`として、Host、Client、Server、Backendの役割へ分解する |
...
| 3 | Flow promise | 「何を順に見るか」が弱い | 全体の流れを判断軸の表に変更 |
...
今回の追加レビューでは、Referencesを除く本編74ページを対象に、各ページの内容抜粋と前後の流れを確認した。問題は前半の概念順ではなく、後半が「開発向けMCPの具体例」から「今後の争点」へ移るときに、何を最終判断として回収するのかが見えにくい点だった。
...
| 5 | 持ち帰り。外部操作、安全性、定型化、制限を述べる。 | OK。ただし最後でこの4点を回収しないと散る。 | 締めで「任せる/見せる/守る/運用する」に再集約。 |
| 6 | 失敗CIを調べる通し例。 | OK。抽象概念を比較章へつなぐ役割。 | 維持。 |
| 7 | 全体の流れ。ピースを順に埋める。 | OK。初学者が章の意味を予測できる。 | 維持。 |
...
| 71 | 旧: 組織内導入の進め方。 | P2。特定の場を前提にした文脈に見える。 | 「導入の進め方」に変更し、一般的な導入手順へ。 |
...
直近の見直しでは、00章以外の章についても「章に入った瞬間に何を見ているか分かるか」「前章から低レイヤ詳細へ急に落ちていないか」を確認した。

| Chapter | 違和感 | Why it blocks beginners | Reflected change |
|---:|---|---|---|
| 02 比較と判断軸 | `なぜMCPが必要なのか`が、00章と01章の定義を繰り返して見える | 読者はすでにMCPの位置と定義を見ているため、ここでまた必要性を説明すると章の役割がぼやける | `どの入口を選ぶべきか`に変更し、CLI / Browser / MCPを選択肢として並べた |
| 03 MCP server構築 | 比較章から急にFastAPI/FastMCPのコードに入る | 初学者が「このコードを覚える章」と誤解し、adapter設計の章だと分かりにくい | `構築章で見る境界`を追加し、既存API / MCP adapter / route policyを先に分けた |
| 04 Remote MCP | localからRemoteへ進む意味は章扉と最初の比較表で説明できている | ここは維持でよい。具体設定に入る前にlocal/Remote差分が置かれている | 変更なし |
| 05 Protocol / Auth / JSON-RPC | transport表から始まるため、Remote接続・認証・JSON-RPC・tool callの話が混ざって見える | transport、auth、message、model tool callは別レイヤーなのに、同じ仕様詳細に見える | `この章で見る3層`を追加し、Transport / Auth / JSON-RPCを分けてから詳細へ入る |
| 06 AWSケーススタディ | AWS製品紹介に見えるリスクはあるが、`なぜAWSを例に見るのか`で位置づけを説明している | Remote MCPの運用責務を具体化するケースとして読める | 変更なし |
| 07 開発ワークフロー | `具体MCPは3分類で見る`というタイトルが、固有MCP紹介に見える | 章の目的はツール名暗記ではなく、作業面で接続面を選ぶこと | `作業面で分類する`へ変更し、固有tool名は代表例として読むようにした |
| 08 ガバナンスと導入 | 前章の具体例を導入判断へ戻す役割が明確 | `導入前に決める5つの判断`から始まり、締めへ戻る流れは自然 | 変更なし |

#### ゼロベース読み手レビュー

「MCPを初めて聞くが、LLMやAPIはある程度知っている」読者として、前置きなしに細かい固有ツールへ入っていないかを再確認した。前回のレビューは終盤の締めに寄りすぎていたため、今回は章間の接続と、具体ツールの説明に入る必然性を重点的に見た。

| Page | Issue | Why it blocks beginners | Change |
|---:|---|---|---|
| 27 -> 28 | 構築章は比較章から自然に入れるが、code例へ行く前に「なぜ実装を見るか」が弱い | 読者がFastAPI/FastMCPの話をMCP全体の必須知識と誤解しやすい | P28-P30で「既存APIをagent向けadapterにする」流れを先に置き、codeは実装例として読む構成を維持 |
| 36 -> 37 | 旧構成ではClaude接続コマンドから入り、Remote MCPの意味より手順暗記に見えた | Remote MCPがなぜ出てきたのか分からないまま、特定clientの設定に入る | P36 subtitleを「チームで共有する接続面へ進む」に変更し、P37をlocal stdioとRemote MCPの比較に差し替え |
| 38 -> 40 | OAuth、project/user/org、client差分が続く | 初学者には「設定ファイルの差分」ではなく「共有能力と個人認証を分ける話」だと先に知る必要がある | P37でremote化の意味を先に置き、P38以降は権限・設定分離の細部として読む |
| 41 -> 52 | Protocol/Auth/JSON-RPCが細かい | 仕様詳細に急に入ったように見える可能性がある | Remote章の後に置き、「Remoteを安全に動かす中身を見る章」として維持。P43-P46で構成図とstepを先に置く |
| 53 -> 54 | 旧構成ではAWS MCP / AgentCoreの説明が急に始まった | 読者が「なぜAWSの製品紹介が始まったのか」と感じる | P53 subtitleとP54を差し替え、AWSは製品紹介ではなくRemote MCPの運用責務を具体化するケースと明記 |
| 55 -> 57 | Gateway / Identity / target / authが続く | AWS用語を知らない読者には、どの概念の写像か分かりにくい | P54でGateway=入口、Identity=credential境界、Target=接続先、Operation=監査/権限と前置きした |
| 58 -> 60 | 旧構成ではWebMCPから入り、Figma、Browser、Serenaへ続いて個別ツール紹介に見えた | 「なぜこのツール群がMCP概論に必要か」が分からない | P58 subtitleを「開発タスクごとの接続面として見る」に変更し、P59にbackend/service、live browser/frontend、design/code contextの分類表を追加 |
| 60 -> 67 | WebMCP、Playwright、Figma、Serenaが連続する | 固有名詞の列挙に見え、各ツールの価値がMCPのどの理解に対応するか見失う | P59の分類に対応させ、WebMCP=live browser、Browser MCP=検証、Figma=design context、Serena=code contextとして読む |
| 68 -> 74 | 具体例からガバナンスへ戻る | 具体ツールの後に急に統制論へ飛ぶように見える | P69で「導入前に決める5つの判断」に戻し、具体ツール選定を対象業務/公開面/権限/信頼/運用予算へ回収する |

#### Revised narrative

現行スライドは、次の読み順になるよう調整した。

1. **概念地図**: MCPの部品を暗記せず、User -> Host -> Server -> Backendの流れを見る。
2. **基本概念**: Host/Client/Server、公開面、通信面、Skillとの違いを揃える。
3. **判断軸**: CLI、Browser、MCPを同じCI調査例で比較し、MCPを選ぶ条件を作る。
4. **構築例**: 既存APIをadapter化する。FastAPI/FastMCPは必須知識ではなく、1つの具体例。
5. **Remote化**: local toolからteam shared serviceへ広がると、認証、scope、project/user/org設定が必要になる。
6. **Protocol/Auth詳細**: Remote MCPを安全に動かすため、transport、auth、JSON-RPC、tool catalogの中身を見る。
7. **AWSケース**: AWSを製品紹介としてではなく、Gateway/Identity/target/operationへ責務分解する例として見る。
8. **開発workflow例**: 個別ツール名ではなく、backend/service、live browser/frontend、design/code contextの接続面として分類する。
9. **導入判断**: 具体例を、何を任せるか、何を見せるか、どこで守るか、どう運用するかへ回収する。
